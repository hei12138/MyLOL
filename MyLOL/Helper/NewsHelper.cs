...
namespace MyLOL.Helper
{
    public class NewsHelper
    {
        ...
        public static async Task<string> GetNews()
        {
            ...
            try
            {
                await client.GetAsync(new Uri("http://example.com")).AsTask(source.Token);
            }
            catch (TaskCanceledException ex)
            {
            ...
            try
            {
                var json2 = await client.GetBufferAsync(new Uri("http://qt.qq.com/static/pages/news/phone/c12_list_1.shtml"));
                //将ibuffer转变为byte[];
